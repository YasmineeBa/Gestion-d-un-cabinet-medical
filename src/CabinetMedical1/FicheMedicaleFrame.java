...
public class FicheMedicaleFrame {
	...
	private void initialize() {
		...
		try{
			...
			ResultSet rs = stmt.executeQuery(sql);
			if(rs.next()){//dans le cas où le patient est diabétique on ajoute le champs taux diabete
...
textGrpSanguin.setText(" ");
 ...
 refreshChamps();//actualiser les champs contenant l'état de santé du patient 
	...
	comboBox.addItem("O-"); comboBox.addItem("O+"); comboBox.addItem("AB-"); comboBox.addItem("AB+");
	 ...
	 btnEnregistrerResltat.addActionListener(new ActionListener() {
	 	public void actionPerformed(ActionEvent arg0) {
	 		OpeningAnalyse=table.getValueAt(table.getSelectedRow(),0).toString(); //selectionné l'analyse
	 		frmFicheMdicale.dispose();
	 		EnregistrerResultatFrame.main(null); //appeler la fênetre qui enregistre les résultat de l'analyse
	 	}
	 });
 ...
 btnEnregistrerLesModifications.addActionListener(new ActionListener() {
 	public void actionPerformed(ActionEvent arg0) {
 		...
 		if (action==0) {//confirmation obtenu
			...
			EnregistrerResultatFrame.EnregistrerModifications();//enresgitrer le resultat des analyses modifiées
			frmFicheMdicale.dispose();
		}
 ...
 btnEnregistrerLesModifications.setIcon(imgSave);
	 ...
	 public void refreshChamps(){//Initialisation des champs / acualisation
			FicheMedicale f=FicheMedicale.ChercherFicheMedicale(numFiche);//recherche de la fiche médicale
			if(f!=null){
				//initialisation
